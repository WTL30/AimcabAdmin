<?php 
   ...
   $timeend = date('h:i a', strtotime($timeend1));
    ...
    //VALUES ('$pickup', '$drop', '$date', '$time', '$phone', '$trip', '0', '$bookid', '$distance', '$car', '$new_amount', '$partial', '$dateend', '$timeend', '$bookingtype', '$name', '$email', '$totalpaidAmt','$remainAmt')";
     ...
     $sql ="INSERT INTO `custom_booking` (user_pickup, user_drop, date, time,extrahours,extrahoursrs,extrakm,extrakmrs,hoursamount,kmamount, phone, user_trip_type, days,status, bookid, distance, car, base,amount, partial, dateend, timeend, bookingtype, name, email, gst, service_charge,driver_bhata, parking,toll,totalpaid_amt, remain_amt, companyname, companyaddress, gstno)
    VALUES ('$pickup', '$drop', '$date', '$time', '$extrahours','$extrahoursrs','$extrakm','$extrakmrs','$hoursamount','$kmamount','$phone', '$trip','$days', '0', '$bookid', '$distance', '$car', '$amount','$new_amount', '$partial','$dateend', '$timeend', '$bookingtype', '$name', '$email','$gst','$service','$driver','$parking','$toll','$totalpaidAmt','$remainAmt', '$companyname', '$companyaddress', '$gstno')";
    
    if ($link->query($sql) != TRUE) {
    echo "<br>" . $link->error;
    }else{
         if($trip == "Rental"){
      header('location:rental-invoice-wtl.php');
        
    }elseif($trip == "One Way" || $trip == "Round"){
        header('location:invoice-wtl.php');
        
  }elseif($trip == "One Way NoGST" || $trip == "Round NoGST"){
      header('location:invoice-withoutgst-wtl.php');
       
  }elseif($trip == "Rental NoGST"){
      header('location:invoice-rentalnogst-wtl.php');
  }
    }
