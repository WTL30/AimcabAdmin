            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        ...
                        include "config.php";
                        $value2 = '';
                        $query = "SELECT bookid FROM custom_booking WHERE bookid LIKE 'AIM%' ORDER BY id DESC LIMIT 1;";
                        $stmt = $link->query($query);
                        if (mysqli_num_rows($stmt) > 0) {
                            // Fetch the last inserted bookid
                            if ($row = mysqli_fetch_assoc($stmt)) {
                                $value2 = $row['bookid'];
                                if (preg_match('/^AIM1000(\d+)$/', $value2, $matches)) {
                                    $numericPart = (int) $matches[1];
                                    $numericPart++;
                                    $value2 = "AIM1000" . sprintf('%01d', $numericPart);
                                } else {
                                    $value2 = "AIM10001";
                                }
                            }
